...
- `src/llmStatus.ts` — `GET /api/v1/llm/status` plus the label helpers `llmErrorLabel` and `llmProviderLabel`. Nothing probes on a render, so a provider with lifetime spend but no recorded outcome reads "No status yet — last call …", not "No calls yet". The list is sorted **troubled-first**. **The Probe button is the one active check** (shallow — a screen never spends tokens); its results are screen state on purpose, so a re-navigation clears them (an e2e check that re-navigates will see nothing — read in place).
